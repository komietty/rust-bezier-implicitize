...
use rand;
use crate::bezier_implicitizer::{calc_deviation, implicitize_bezier};
...
fn quadratic_test() {
    ...
    let root = BitMapBackend::new("quadratic.png", (480, 480)).into_drawing_area();
    root.fill(&bg).unwrap();
    ...
    for t in 0..2 {
        ...
        chart
            .draw_series(LineSeries::new(
                it.clone()
                    .map(|x| (x as f32, (-l1.x / l1.y * x - l1.z / l1.y) as f32)),
                &BLUE,
            ))
            .unwrap();
    }
    ...
    for i in -100..100 {
        let t = i as f64 * 0.1;
        assert!(detA(bezier_curve(vec![p0, p1, p2], t)).abs() < 1e-10);
    }

    let m1 = implicitize_bezier(&vec![p0, p1, p2]);
    let m2 = implicitize_bezier(&vec![p3, p4, p5]);

    {
        ...
        for _ in 0..10000 {
            ...
            if calc_deviation(&m1, p).unwrap().abs() < 0.02
            //&&
